      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span type="text" v-text="scope.row.payId" @click="openeditwin1(scope.row.payId)"></span>
          </template>
        </el-table-column>
        <el-table-column
          prop="reason"
        ...
        <el-table-column
          ...
          <template scope="scope">
            <span v-if="scope.row.payTag === 'K002-1'">执行</span>
            <span v-else-if="scope.row.payTag === 'K002-2'">完成</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
      ...
      <div>
        <el-form  size="small" :inline="true" v-model="scellform">
          <el-row>
            ...
            <el-col :span="18"><h3>出库单查询</h3></el-col>
            <el-col :span="3">
    ...
    export default {
      ...
      methods: {
        ...
        stoQueryByIdSGatherEx(id) {//获取数据
          ...
          var params = new URLSearchParams();
          params.append("id", id);
          this.$axios.post("SPayDetails/queryByIdSPayEX.May", params).then(function (response) {
        ...
        stoQueryByIdSGatherEx2(id) {//获取数据
          ...
          var params = new URLSearchParams();
          params.append("id", id);
          this.$axios.post("SPayDetails/queryByIdSPayEX2.May", params).then(function (response) {
            _this.dfileform = response.data;
            for (var i=0;i<_this.dfileform.length;i++) {
              _this.scellAmount = parseInt(_this.dfileform[i].paidAmount*1) * parseInt(_this.dfileform[i].costPrice*1);
